...
const consume_frame_stream = function(objects,keys,types,length) {
  ...
  return Promise.all( outputs.map( (output) => output.promise )).then(function() {
    ...
    return new Promise(function(resolve,reject){
      async.eachOfSeries(filenames,function(file,idx,callback) {
        console.log(file);
        let readstream = fs.createReadStream(file);
        readstream.on('data', (dat) => { console.log("got dat ",keys[idx],dat); self.stream.write(dat) });
        readstream.on('end', () => { console.log("Done reading file "); callback() });
      },function(err) {
        if (err) {
          reject(err);
        } else {
...
const dataFrame = function(object,keys,types,length) {
  ...
  if (object.next) {
    return (consume_frame_stream.bind(self))(object,keys,types,length).then(function() {
  ...
  if (value instanceof Stream.Readable && ! value._readableState.objectMode) {
    ...
    return new Promise(function(resolve,reject) {
      value.on('end',function() {
        console.log("Ended stream for ",type);
        resolve();
      });
