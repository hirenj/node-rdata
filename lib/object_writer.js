...
const consume_frame_stream = function(objects,keys,types,length) {
  let outputs = keys.map(function(key,idx) {
    ...
    let output = new ObjectWriter(fs.createWriteStream('tempfile'));
    let promise = output.writeValue(stream,types[idx]);
    output.promise = promise;
    output.instream = stream;
    return output;
  });
  objects.on('data',function(row) {
...
const dataFrame = function(object,keys,types,length) {
  ...
  if (object.next) {
    return consume_frame_stream(object,keys,types,length);
  }
  return new Promise(function(resolve) {
