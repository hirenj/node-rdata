...
const writeValue = function(value,type,length) {
  ...
  let value_written = Promise.reject(new Error("No valid data type given"));
...
const consume_frame_stream = function(objects,keys,types,length) {
  ...
  let outputs = keys.map(function(key,idx) {
    ...
    let promise = writeValue.call(output,stream,types[idx],length);
  ...
  return Promise.all( outputs.map( (output) => output.promise )).then(function() {
    ...
    return self.dataFrame(frame,keys,types,length ? length : counter.total);
  });
};
...
const dataFrame = function(object,keys,types,length) {
  ...
  return new Promise(function(resolve,reject) {
    async.eachOfSeries(keys,function(column,idx,callback) {
      writeValue.call(self,object[column],types[idx],fixed_length).then(callback);
    }, function(err) {
      if (err) {
        reject(err);
      } else {
        length.then((length_val) => {
          ...
          let attributes = { "names": keys, "class" : ["data.frame"], "row.names" : [-1*(0x80000000),-1*length_val] };
          return self.listPairs(attributes,[ "names","row.names","class" ] ,["string","int","string"]);
        }).then(resolve);
      }
    });
  });
